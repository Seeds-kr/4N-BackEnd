        ...
        if not (useremail and password):
            ...
            try:
                ...
                if check_password(password, user.password):
                    ...
                    return JsonResponse({"message": '로그인 했습니다.', "username": user.username}, status=200)
                else:
...
def logout(request):
    if request.method == "GET":
        if request.session.get('user_id') == None:
            return JsonResponse({"message": "로그인이 안 되었습니다."}, status=400)
...
def findid(request):
    if request.method == "POST":
        ...
        return JsonResponse({"message": "유저 아이디를 찾았습니다.", "useremail": useremail}, status=200)
...
class KakaoLoginView(APIView):
    def get(self, request):
        ...
        try:
            ...
            print(e)
            return Response({"message": "카카오 로그인 오류가 발생했습니다."}, status=400)
    ...
    def post(self, request):
        ...
        try:
            ...
            if logout_response.status_code == 200:
                ...
                return Response({"message": "카카오 로그아웃 오류가 발생했습니다."}, status=400)
        ...
        try:
            ...
            try:
                ...
                if Naver_users.exists():
                    ...
                    user.save()
                    request.session['Naver_user'] = user.id
                    return Response({"message": "간편 회원가입이 완료되었습니다."}, status=200)
